$(document).ready(function() {
    ...
    function lookDelete() {
        $.post('' + http_head + '/Articles/Get_Article_Recycle.ashx', {
            "userGuid": userGuid
        }, function(data) {
            ...
            function optionFormatter(e, value, row, index) {
                return ['<a class="delete"  title="彻底删除">', '<span id="delete">彻底删除</span>', '</a>  ', '<a class="recovery"  title="恢复文章">', '<span id="recovery">恢复文章</span>', '</a>  ', '<a class="more"  title="查看详情">', '<span id="more">查看详情</span>', '</a>  ', ].join('');
            };
        });
    }
    lookDelete()
    ...
    window.optionEvents = {
        'click .delete': function(e, value, row, index) {
            ...
            $('.layui-layer-btn0').click(function() {
                $.post('' + http_head + '/Articles/Delete_ArticleRecycle.ashx', {
                    "userGuid": userGuid,
                    "articleId": row.Id
                }, function(data) {
        ...
        'click .recovery': function(e, value, row, index) {
            ...
            $('.layui-layer-btn0').click(function() {
                $.post('' + http_head + '/Articles/Article_Recovery.ashx   ', {
                    "userGuid": userGuid,
                    "articleId": row.Id
                }, function(data) {
