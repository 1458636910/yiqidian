$(document).ready(function() {
		...
		if (data.status == '200') {
			...
			$("#reporting").bootstrapTable({
				...
				$.ajax({
					...
					data:{userGuid:arr_3,key:field,value:row[field],Id:row.Id},
					dataType: 'JSON',
	...
	$('#ficient').click(function() {
		if ($('#EditText_1').val() == '') {
			layer.msg('请输入！', {
				time: 1000,
			});
		} else {
			$.post('http://47.94.173.253:8008/WeekPlan/buZu_add.ashx', {
				userGuid: arr_3,
				companyId: arr_4,
				buZu: $('#EditText_1').val(),
				weekId: weekId
	...
	$('#Advantage').click(function() {
		if ($('#EditText_2').val() == '') {
			...
			$.post('http://47.94.173.253:8008/WeekPlan/youdian_add.ashx', {
				userGuid: arr_3,
				companyId: arr_4,
				youDian: $('#EditText_2').val(),
				weekId: weekId
			}, function(data) {
				var data = JSON.parse(data);
				if (data.status == '200') {
	...
	$('#Sentiment').click(function() {
		if ($('#EditText_3').val() == '') {
			...
			$.post('http://47.94.173.253:8008/WeekPlan/ganwu_add.ashx', {
				userGuid: arr_3,
				companyId: arr_4,
				ganWu: $('#EditText_3').val(),
				weekId: weekId
			}, function(data) {
				var data = JSON.parse(data);
				if (data.status == '200') {
	...
	$('.btnd').click(function(){
		if($('#EditText_1').val() == '' || $('#EditText_2').val() == '' || $('#EditText_3').val() == ''){
			layer.msg('请填写批评与自我评价内容！', {
				time: 1000,
			});
		}else{
		layer.open({
			title: ['发送周总结', 'font-size:16px;text-align: center;'],
			...
			yes: function(index, layero) {
				...
				$('#Advantage').click();
				$('#Sentiment').click();
				$.post('http://47.94.173.253:8008/WeekPlan/weekId/weekSend.ashx', {
					userGuid: arr_3,
					weekId: weekId
				}, function(data) {
					data = JSON.parse(data);
