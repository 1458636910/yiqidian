$(document).ready(function() {
	$(".mainFountion li a").eq(6).unbind('mouseenter').unbind('mouseleave').css({
		"color":"#00a2e8"
	});
	...
	$.post('' + http_head + '/Training/Get_trainDemDetail.ashx', {
		"tdid": videoDetails
	}, function(data) {
		dataDemDetails = JSON.parse(data);
		
		// 判断是否结束
		var yourtime = dataDemDetails.items.thedateEnd.split("T")[0]
		var addSpan;
		...
		$('.needTitle').text(dataDemDetails.items.theTheme);
		$('.demandTime').text(dataDemDetails.items.thedateStart.split("T")[0] + "~" + dataDemDetails.items.thedateEnd.split(
			"T")[0]);
		$('.needLabel').text(dataDemDetails.items.theCategory);
		$('.creatTime').text(dataDemDetails.items.createTime.split("T")[0]);
		$('.keywords').text(dataDemDetails.items.keywords);
		$('.budgetedExpense').text(dataDemDetails.items.budgetedExpense);
		...
		$('.lecturerAge').text(dataDemDetails.items.lecturerMinAge + '~' + dataDemDetails.items.lecturerMaxAge + '岁');
		$('.lecturerShuXiang').text(dataDemDetails.items.lecturerShuXiang);
		$('.lecturerJinJIShuXiang').text(dataDemDetails.items.lecturerJinJIShuXiang);
		$('.lecturerAddress').text(dataDemDetails.items.lecturerAddress);
		$('.lecturerEducation').text(dataDemDetails.items.lecturerEducation);
		$('.lecturerMagor').text(dataDemDetails.items.lecturerMagor);
		$('.lecturerZhiCheng').text(dataDemDetails.items.lecturerZhiCheng);
		$('.lecturerLanguages').text(dataDemDetails.items.lecturerLanguages);
		$('.lecturerWorkBackground').text(dataDemDetails.items.lecturerWorkBackground);
		$('.lecturerPost').text(dataDemDetails.items.lecturerPost);
		var time1 = (dataDemDetails.items.thedateStart).split("T")[0];
		var time2 = (dataDemDetails.items.thedateEnd).split("T")[0];
		var cTime = (dataDemDetails.items.createTime).split("T")[0];
		$('.time').text(time1 + '~' + time2)
		$('.lanchTime').text(cTime)
		// 本类其他需求
		loadSameDemand(dataDemDetails.items.theCategory)
