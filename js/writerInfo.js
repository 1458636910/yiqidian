$(document).ready(function(){
  ...
  $.post('https://www.eqid.top:8009/Articles/Get_AuthorInfo.ashx ', {
      ...
      "authorGuid":uGuid
          ...
          console.log(data)
          var writeData = JSON.parse(data).items;
          $('.nav .writerDetails img').attr('src',writeData.headimage );
          $('.nav .writerDetails .wName').text(writeData.upname );
          $('.nav .writerDetails .wcircleTips span').text(writeData.articleCount );
          $('.nav .writerDetails .wfoucedTips span').text(writeData.attentionCount );
          $('.nav .writerDetails .wfansTips span').text(writeData.fansCount );
          if ( writeData.isAttent === true ) {
            $('.nav .writerDetails .fouceBtn .fouced').show();
          }else{
             $('.nav .writerDetails .fouceBtn .nofouce').show();
              $('.nav .writerDetails .fouceBtn .nofouce').click(function() {
                $.post('https://www.eqid.top:8009/Articles/Add_Article_Attention.ashx', {
                  "attention":uGuid,
                  "userGuid":dataInfo.Guid
                }, function(data) {
                  var fouceSuss = JSON.parse(data);
                  if (fouceSuss.status == 200) {
                    layer.msg('关注成功', {
                                               time: 1000,
                                               });
                    $('.nav .writerDetails .fouceBtn .nofouce').hide();
                    $('.nav .writerDetails .fouceBtn .fouced').show();
                  }
                });
              });
          }
          $('.nav .writerDetails .wfoucedTips').click(function() {
            window.open("../html/writerFouced.html")
          });
          $('.nav .writerDetails .wfansTips').click(function() {
  ...
  $.post('https://www.eqid.top:8009/Option_AreasAnd.ashx', {type: 44}, function(data) {
            ...
            $('.mainLeft p').hover(function() {
                  ...
                  $('#'+tId+'').css('background-color', 'rgba(0,0,0,0.3)').siblings('p').css('background-color', ' #d4d4d4');
            }, function() {
            });
            ...
            $('.main').hover(function() {
              ...
              $('.list').hide();
              $('.mainLeft p').css('background-color', ' #d4d4d4')
            });
            $('.img').hover(function() {
              $('.list').hide();
            }, function() {
              ...
              dataX = JSON.parse(data).items;
                   ...
                   for (var i = 0; i < dataX.rows.length; i++) {
                       ...
                       if ( dataX.rows.length == 10 ) {
                        $('#circleListHot').append('<div class="loadMore"><a>阅读更多</a></div>')
                       $('.loadMore').click(function() {
                         window.open("../html/writerCircle.html?user="+uGuid+"")
                       });
                     }else{
                      $('#circleListHot').append('<div class="noloadMore"><a>暂无更多</a></div>')
                     }
                     })
  });
              ...
              dataY = JSON.parse(data).items;
                   ...
                   for (var i = 0; i < dataY.rows.length; i++) {
                       ...
                       $('#circleListGood').children('div').children('.label').children('span').click(function() {
                        ...
                        if ( dataX.rows.length == 10 ) {
                           $('#circleListGood').append('<div class="loadMore"><a>阅读更多</a></div>')
                           $('.loadMore').click(function() {
                         window.open("../html/writerLikedCircle.html?user="+uGuid+"")
                       });
                         }else{
                          $('#circleListGood').append('<div class="noloadMore"><a>暂无更多</a></div>')
                         }
                     })
  });
  // $('#fans').click(function() {
  ...
  //                       // 查看作者详情
  //                       $('#circleListGood').children('div').children('.writetInfo ').children('img ').click(function() {
  //                         window.open("../html/writerInfo.html?user="+$(this).attr('id')+"")
  //                       });
  //                       if ( dataX.rows.length == 10 ) {
  //                          $('#circleListGood').append('<div class="loadMore"><a>阅读更多</a></div>')
  //                          $('.loadMore').click(function() {
